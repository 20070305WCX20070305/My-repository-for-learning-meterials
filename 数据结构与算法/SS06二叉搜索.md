...
class BinarySearchTree:
    ...
    def _size(self, node):
        return 0 if not node else 1 + self._size(node.left) + self._size(node.right)
    ...
    def _delete(self, node, val):
        ...
        if val < node.val:
            ...
            if (not node.left) and (not node.right):
                ...
                return node.left if not node.left else node.right
    ...
    def order(self, mode = "pre"):
        res = []
        self._preorder(self.root, res, mode)
        return res
